...
class ImageUploadingHelper
{
    ...
    private static $storage_url= 'https://amen-project.s3.us-east-2.amazonaws.com/';
    ...
    //   $destinationPath = ImageUploadingHelper::real_public_path() . $destinationPath;
      ...
      Storage::disk('s3')->put($destinationPath.'/'.$fileName,file_get_contents($field));
//        dd($fileName);
//        /*         * **** Resizing Images ******** */
...
//        }
    ...
    public static function print_image($image_path, $width = 0, $height = 0, $default_image = '/admin_assets/no-image.jpeg', $alt_title_txt = '')
    {
    ...
    public static function get_image($image_path, $width = 0, $height = 0, $default_image = '/admin_assets/no-image.jpeg', $alt_title_txt = '')
    {
		$dimensions = '';
        if ($width > 0 && $height > 0) {
	...
	public static function print_image_src($image_path, $width = 0, $height = 0, $default_image = '/admin_assets/no-image.jpeg', $alt_title_txt = '')
    {
     $exists = Storage::disk('s3')->has($image_path) ;
     //file_exists(ImageUploadingHelper::real_public_path() . $image_path)
        if (!empty($image_path) && $exists) {
           // return ImageUploadingHelper::public_path() . $image_path;
            return self::$storage_url . $image_path;
        } else {
    ...
    public static function print_image_relative($image_path, $width = 0, $height = 0, $default_image = '/admin_assets/no-image.jpeg', $alt_title_txt = '')
    {
        $dimensions = '';
        if ($width > 0 && $height > 0) {
            $dimensions = 'style="max-width=' . $width . 'px; max-height:' . $height . 'px;"';
