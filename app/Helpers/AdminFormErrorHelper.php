...
class AdminFormErrorHelper
{
    ...
    public static function showErrorsNotice($errors)
    {
        $html = '';
        if (count($errors) > 0) {
            $html .= '<div class="alert alert-danger"> '. __('You have some form errors. Please check below.') . '<ul>';
    ...
    public static function showOnlyErrorsNotice($errors)
    {
        $html = '';
        if (count($errors) > 0) {
            $html = '<div class="alert alert-danger"> '. __('You have some form errors. Please check below.') . '</div>';
        }
        return $html;
    }
