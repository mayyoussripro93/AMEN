...
@section('content')
    ...
    <div class="page-content-wrapper">
        ...
        <div class="page-content">
            ...
            <!-- END PAGE BAR -->
            <!-- BEGIN PAGE TITLE-->
            <h3 class="page-title">Manage Employees <small>Employees</small> </h3>
            <!-- END PAGE TITLE-->
            <!-- END PAGE HEADER-->
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="portlet light portlet-fit portlet-datatable bordered">
                        <div class="portlet-title">
                            <div class="caption"> <i class="icon-settings font-dark"></i> <span class="caption-subject font-dark sbold uppercase">Employees</span> </div>
                            <div class="actions">
                                <a href="{{ route('create.employee') }}" class="btn btn-xs btn-success"><i class="glyphicon glyphicon-plus"></i> Add New Employee</a>
                            </div>
                        </div>
    ...
    <script>
        $(function () {
            var oTable = $('#user_datatable_ajax').DataTable({
                 ...
                 */
                ajax: {
                    url: '{!! route('fetch-employees-delete-request') !!}',
                    data: function (d) {
        ...
        function make_verified(id) {
            $.post("{{ route('make.verified.employee') }}", {id: id, _method: 'PUT', _token: '{{ csrf_token() }}'})
                .done(function (response) {
                    if (response == 'ok')
                    {
        ...
        function make_not_verified(id) {
            $.post("{{ route('make.not.verified.employee') }}", {id: id, _method: 'PUT', _token: '{{ csrf_token() }}'})
                .done(function (response) {
                    if (response == 'ok')
                    {
